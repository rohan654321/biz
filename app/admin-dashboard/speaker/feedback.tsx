...
export default function SpeakerFeedbackPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      setActionLoading(true)
      const response = await fetch(`/api/admin/speaker-feedback/${selectedFeedback.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "reject",
  ...
  const filteredFeedback = feedback.filter((item) => {
    const matchesSearch =
      ...
      item.speakerEmail.toLowerCase().includes(searchQuery.toLowerCase())
                  ...
                  <TableRow key={item.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">{item.speakerEmail}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div>
                        ...
                        {item.sessionTitle && <div className="text-xs text-gray-500">{item.sessionTitle}</div>}
                      </div>
                    </TableCell>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">
                    <span className="text-gray-600">Name:</span> {selectedFeedback.speakerName}
                  </p>
                  <p className="text-sm">
                    <span className="text-gray-600">Email:</span> {selectedFeedback.speakerEmail}
                  </p>
                </div>
                <div>
      ...
      <Dialog open={isRejectOpen} onOpenChange={setIsRejectOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Please provide a reason for rejecting this feedback.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <Textarea
