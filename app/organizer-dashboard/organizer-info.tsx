...
export default function OrganizerInfo({ organizerData: initialData }: OrganizerInfoProps) {
  ...
  const handleSave = async (section: string) => {
    try {
      setLoading(true)
      const response = await fetch(`/api/organizers/${organizerData.id}`, {
        method: "PUT",
        headers: {
      ...
      toast({
        title: "Error",
        description: "Failed to update organizer information",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setIsEditing(isEditing === "specialties" ? null : "specialties")}
            >
              <Edit className="w-4 h-4 mr-2" />
            ...
            {organizerData.specialties.map((specialty, index) => (
              <Badge key={index} variant="secondary" className="flex items-center gap-1">
                ...
                {isEditing === "specialties" && (
                  <X className="w-3 h-3 cursor-pointer hover:text-red-500" onClick={() => removeSpecialty(index)} />
                )}
              </Badge>
            ))}
          </div>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setIsEditing(isEditing === "achievements" ? null : "achievements")}
            >
              <Edit className="w-4 h-4 mr-2" />
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => setIsEditing(isEditing === "certifications" ? null : "certifications")}
            >
              <Edit className="w-4 h-4 mr-2" />
